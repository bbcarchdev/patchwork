...
static int patchwork_query_db_media_(struct db_qbuf_struct *qbuf, struct query_struct *query);
...
static int
patchwork_query_db_media_(struct db_qbuf_struct *qbuf, struct query_struct *query)
{
	size_t i;
	...
	if(query->media)
	{
		...
		else if(!patchwork_array_contains(query->audience, "any"))
		{
			...
			{
				quilt_logf(LOG_DEBUG, QUILT_PLUGIN_NAME ": patchwork_query_db_media_ adding audience %s'\n", query->audience[i]);
...
patchwork_item_db(QUILTREQ *request, const char *id)
{
	 ...
	 *
	 * Finally, we can invoke patchwork_item_db_render_() to populate the
	...
	patchwork_item_db_render_(&item);
	if(rs)
	{
		sql_stmt_destroy(rs);
	}
...
int
patchwork_lookup_db(QUILTREQ *request, const char *target)
{
	SQL_STATEMENT *rs;
	const char *t;
	char *buf, *p;
...
int
patchwork_audiences_db(QUILTREQ *request, struct query_struct *query)
{
	SQL_STATEMENT *rs;
	int limit, offset;
	QUILTCANON *dest;
...
int
patchwork_item_db_render_(struct db_item_struct *item)
{
	char *subj;
	QUILTCANON *selfc;
